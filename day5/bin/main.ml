...
(* Part 1*)
type command = { amount : int; from : int; onto : int }
...
let split_every_n n str =
  ...
  let rec aux i acc =
    if i >= length then List.rev acc
      ...
      aux (i + n) (String.sub str ~pos:i ~len:(min (n - 1) (length - i)) :: acc)
  in
  aux 0 []
...
let parse_stack stack =
  ...
  |> List.transpose_exn |> Array.of_list
...
let () =
  ...
  let commands =
    List.last_exn input |> String.split_lines |> List.map ~f:parse_command
  in
